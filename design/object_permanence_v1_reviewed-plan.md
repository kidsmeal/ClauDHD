...
- **As-built amendments (phase-5):** the popover is its own always-on-top tauri window (label `capture`, hidden at rest) booted by `src/ui/capture-window.ts` when `getCurrentWindow().label === "capture"` (plan named it views/capture.ts); the IDEAS.md insert lives in the courier as `idea_append` (mirrors idea.js exactly: newest-first under `## Inbox`, `(empty)` placeholder removed, header created when missing, the same `.now/ideas.lock` held so a live session capture can never be clobbered, atomic temp+rename, refuses non-claudhd targets and multiline entries); pure line/tokenizer logic is `src/core/capture.ts`; launcher spawns exe+args directly (`launch_detached`, no shell) so a missing executable fails fast and is reported; settings is a read-only config surface with an open-data-folder action (in-place editing deferred; the file is the editor, config reads at boot); launcher tests folded into `test/capture.test.ts`; `data_dir_path` courier command added for the explorer button. Live proof lesson baked into capture-window.ts: keydown listens on `document` (keystrokes target body before the input claims focus, and body is the root div's parent), and element focus retries on a short timer because a synchronous .focus() inside the window-focus event noops in the webview. Wiring touchpoints (phase-5 review): `src/main.ts` (hotkey registration with armed/not-armed states, capture-window boot path, launch + openDataFolder handlers, per-scan capture-targets.json write), `src/ui/render.ts` (launch/menu/open-folder delegation), `src/ui/store.ts` (hotkeyArmed/launchMenuFor/launchError/config), `test/render.test.ts` (inert-stub assertion flipped to the live launcher). The ideas lock is taken as a DIRECTORY (create_dir/remove_dir), the exact primitive lock.js uses, so stale-lock recovery stays symmetric between the tools after a crash.
