---
description: Loud escape hatch - record an unguarded session so mode enforcement stops denying your edits
allowed-tools: Bash(node:*)
---
!`node "${CLAUDE_PLUGIN_ROOT}/scripts/override.js"`
...
Relay the line above to me verbatim. Then say plainly what this means: every file the override lets through gets logged (the count grows as you go), and the record stays visible in NOW.md until the next `/claudhd:build`, `/claudhd:design`, or a fresh override session replaces it. This is not a way to skip review; it is a way to say out loud "I am working outside any phase right now, on purpose." If you did not mean to run this, tell me and stop working outside the active phase's scope instead.
