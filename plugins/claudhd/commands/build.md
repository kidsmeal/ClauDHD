...
The implementer works tests-first (where a test framework exists), stays inside the plan's file list, will not commit, and will not advance past the one phase. When it returns, keep its full report - it is machine-facing, feeding the review relay and the round log unchanged. Report to me in this shape only:
- line 1: "phase `<phase>` built: `<N>` files created, `<M>` modified, tests `<A>` -> `<B>`"
- a "needs you:" block listing every blocker, unapplied artifact, and open decision from the report verbatim in substance (never compress these away; they are the human gate's input)
- closing line: "full report available if you want it"

Set the phase's `**Status:**` line in the plan to `built` (the implementer never edits the plan; you do). Then stop and recommend `/claudhd:review <plan> <phase>` before I commit. Do not start the next phase, and do not commit.
