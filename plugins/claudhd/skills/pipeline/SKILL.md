...
## Before you start
0. **Preflight the harness.** Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/preflight.js` and wait for it. Exit 0 means this is a Claude Code shell, where the hooks and agent files that enforce the sentinel, scope log, commit reconcile, and per-role tool limits are active. On a non-zero exit, print its message and stop: run no stage. The prose below carries none of those guarantees on its own (on 2026-09-08 it drove a Codex session with no guards and no reconcile). This is the rule `role.js` already applies to the implementer, lifted to the orchestrator.
...
2. **Confirm the project is initialized.** Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/paths.js` once; it prints where this project keeps its ClauDHD files (`NOW=`, `ROADMAP=`, `IDEAS=`, `SHIPPED=`, `AUDIT=`, `RVQ=`, `DESIGN_DIR=`), and every file name in this skill means the file at that path. `CURRENTNESS_AUDIT.md` and `RUNTIME_VERIFICATION_QUEUE.md` should exist and the project's conventions + test command should be known. If not, run `/claudhd:init` first.
...
1. Before spawning the implementer, run and wait for: `node ${CLAUDE_PLUGIN_ROOT}/scripts/sentinel.js write <plan-path> <phase-number>`, then `node ${CLAUDE_PLUGIN_ROOT}/scripts/thread.js enter-build` (display only; the sentinel is what the guard actually enforces against). If the project has not opted in (no `.now/enabled`/`.gantry/enabled`), both are harmless no-ops; run them regardless. Then spawn the **implementer** with the plan path and the phase number. Its report is machine-facing: keep it in full for the review relay and the round log, and report to the user only "phase `<phase>` built: `<N>` files created, `<M>` modified, tests `<A>` -> `<B>`", a "needs you:" block for every blocker, unapplied artifact, and open decision the report contains (verbatim in substance, never compressed away), and a closing "full report available if you want it". Then set the phase's Status to `built`.
...
3. Spawn the **phase-reviewer** with the plan path and phase number, over the uncommitted diff. Handle the Docs impact section: standing docs the diff made stale, each tagged mechanical (fix immediately, plain text edit) or judgment (append to `CURRENTNESS_AUDIT.md`'s `## Open doc flags` for `/claudhd:audit` to reconcile). Report to the user only: a verdict line (PASS / FAIL / PASS-WITH-NOTES), then a "fixes in flight:" line or, on a clean PASS, "commit gate open", then a "needs you:" block only when the 5-FAIL valve triggers or a decision needs the user, then a closing "full report available if you want it".
...
5. **Log deferred notes, then commit gate.** Append every surviving Deferred note to `CURRENTNESS_AUDIT.md`'s `## Deferred review notes` section before presenting for commit; a deferred note that lives only in chat is a dropped note. Then, on PASS or on PASS-WITH-NOTES where all remaining notes are Deferred (now logged), set the phase's Status to `ready to commit`, then run `node ${CLAUDE_PLUGIN_ROOT}/scripts/thread.js commit-policy` and wait for it. It prints `gate` or `auto <plan>`. On `gate`, present the clean diff and verdict and **stop for the user to say go.** On `auto` for this plan (the user granted it once with `thread.js commit-policy auto <plan>`; NOW.md's Position line shows it), present the diff and verdict, run step 6 without asking, and report the commit hash. The grant clears itself when the plan's final phase commits. Push stays a human action under either policy. claudhd 1.0 ships primary-reviewer-only, so the primary reviewer's clean verdict is what opens the gate; there is no second-opinion pass.
...
- Every commit, unless the user granted `commit-policy auto` for this plan (then the commit runs after a clean review without asking; push still waits for the user).
