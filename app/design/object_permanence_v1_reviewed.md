...
3. **Watcher + memory.** fs events, git poll, focus revalidation, event history, since-last-open snapshot and diff, provenance line, tray residency. Snapshot semantics (resolved 2026-07-12): the baseline is "what you last looked at" and writes on focus LOSS only (close-to-tray hides and therefore blurs, so it is covered), plus a one-time boot seed when no baseline exists; polls and fs events never touch it, so unseen drift can never be folded into the baseline. "Last open" anchors to the last time the window gained focus, unchanged. A quit while focused skips the final write and over-reports on the next open, never under-reports. [BUILD-SESSION AMENDMENT 2026-07-12, pending your ratification: the original pin said "writes on close-to-tray and on quit". The phase-3 reviewer FAILED the first implementation because a save-on-every-scan variant let the 120s poll absorb unseen drift into the baseline while tray-resident, hiding it from the next open (the kill-criterion-2 case). The focus-loss-only model above is what shipped; say the word to strike this marker or reverse the paragraph.]
